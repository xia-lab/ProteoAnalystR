...
my.perform.gsea<- function(dataName, file.nm, fun.type, netNm, mType, selectedFactorInx=1, mode = "multi",rankOpt=""){
  ...
  msg("=== DEBUG: my.perform.gsea START ===")
  msg("DEBUG: dataName = ", dataName)
  msg("DEBUG: file.nm = ", file.nm)
  msg("DEBUG: fun.type = ", fun.type)
  msg("DEBUG: netNm = ", netNm)
  msg("DEBUG: mType = ", mType)
  ...
  msg("DEBUG: paramSet$data.org = ", paramSet$data.org)
  msg("DEBUG: paramSet$data.idType = ", paramSet$data.idType)
  msg("DEBUG: paramSet$data.type = ", ifelse(is.null(paramSet$data.type), "NULL", paramSet$data.type))
  msg("DEBUG: First 5 rownames of dataSet$data.norm: ", paste(head(rownames(dataSet$data.norm), 5), collapse=", "))
  msg("DEBUG: Total features in data.norm: ", nrow(dataSet$data.norm))
  ...
  msg("DEBUG: current.featureset loaded - first pathway has ", length(current.featureset[[1]]), " genes")
  msg("DEBUG: current.universe first 5: ", paste(head(current.universe, 5), collapse=", "))

  # CRITICAL: current.universe contains Entrez IDs from pathway database
  # Need to map to symbols for display
  ...
  msg("DEBUG: rankedVec length = ", length(rankedVec))
  msg("DEBUG: First 5 names of rankedVec (BEFORE conversion): ", paste(head(names(rankedVec), 5), collapse=", "))
  msg("DEBUG: First 5 values of rankedVec: ", paste(head(rankedVec, 5), collapse=", "))
  ...
  ranked.ids <- names(rankedVec)
  msg("DEBUG: Converting rankedVec names from UniProt to Entrez for FGSEA matching...")

  # Check if IDs are Entrez-like or UniProt
  is.entrez.like.ranked <- mean(grepl("^[0-9]+$", ranked.ids)) > 0.9
  msg("DEBUG: rankedVec IDs appear to be Entrez-like: ", is.entrez.like.ranked)

  if(!is.entrez.like.ranked) {
    ...
    n.mapped <- sum(!is.na(entrez.ids.ranked))
    msg("DEBUG: Mapped ", n.mapped, " / ", length(ranked.ids), " IDs to Entrez")
    msg("DEBUG: First 5 Entrez IDs: ", paste(head(entrez.ids.ranked, 5), collapse=", "))

    # Remove NAs (unmapped IDs)
    valid.idx <- !is.na(entrez.ids.ranked)
    rankedVec <- rankedVec[valid.idx]
    entrez.ids.ranked <- entrez.ids.ranked[valid.idx]
  ...
  if(fun.type %in% c("go_bp", "go_mf", "go_cc")){
      fgseaRes <- fgsea::fgsea(pathways = current.featureset, 
                          stats    = rankedVec,
                          minSize  = 5,
                          maxSize = 500,
                          scoreType = "std",
  ...
  if(nrow(fgseaRes)<1){
    ...
    netData <- list(sizes=analSet$listSizes, proteinlist=initsbls);
  ...
  if(nrow(fgseaRes) > 20) {
    msg("DEBUG: Limiting GSEA results from ", nrow(fgseaRes), " to top 20 pathways")
    fgseaRes <- fgseaRes[1:20, ]
  }

  current.mset <- current.featureset[fgseaRes$pathway]
  current.mset <- current.mset[!duplicated(names(current.mset))]

  ora.vec <- names(rankedVec)
  msg("DEBUG: Before ID conversion - First 5 ora.vec: ", paste(head(ora.vec, 5), collapse=", "))

  # CRITICAL FIX: Check if IDs are Entrez (all numeric) or UniProt
  # NOTE: After upload, ProteoAnalyst always uses UniProt IDs internally for proteomics
  # paramSet$data.idType refers to the INITIAL upload type, not current internal format
  is.entrez.like <- mean(grepl("^[0-9]+$", ora.vec)) > 0.9
  msg("DEBUG: IDs appear to be Entrez-like (all numeric): ", is.entrez.like)

  if(is.entrez.like) {
    # IDs are already Entrez, use directly
    msg("DEBUG: IDs are already Entrez IDs, using direct Entrez->Symbol mapping")
    entrez.vec <- ora.vec
    ora.nms <- doEntrez2SymbolMapping(entrez.vec, paramSet$data.org, "entrez")
  } else {
    # IDs are UniProt, convert UniProt -> Entrez -> Symbol
    msg("DEBUG: IDs are UniProt format - converting UniProt -> Entrez -> Symbol...")
    ...
    msg("DEBUG: Mapped ", sum(!is.na(entrez.vec)), " UniProt IDs to Entrez")
    msg("DEBUG: After UniProt->Entrez - First 5: ", paste(head(entrez.vec, 5), collapse=", "))

    # Get gene symbols
    ora.nms <- doEntrez2SymbolMapping(entrez.vec, paramSet$data.org, "entrez")
    msg("DEBUG: After Entrez->Symbol - First 5: ", paste(head(ora.nms, 5), collapse=", "))

    # CRITICAL: Update ora.vec to use Entrez IDs for pathway matching
    ora.vec <- entrez.vec
    msg("DEBUG: Updated ora.vec to use Entrez IDs for pathway matching")
  }
  ...
  if(nrow(fgseaRes)<1){
    ...
    # Check if IDs are Entrez (all numeric) or UniProt
    is.entrez.like.list <- mean(grepl("^[0-9]+$", analSet$list.features)) > 0.9

    if(is.entrez.like.list) {
      msg("DEBUG: list.features are Entrez IDs, using direct mapping")
      initsbls <- doEntrez2SymbolMapping(analSet$list.features, paramSet$data.org, "entrez")
    } else {
      msg("DEBUG: list.features are UniProt - Converting to Entrez then Symbol...")
      # Normalize UniProt IDs
      normalized.list <- analSet$list.features
      normalized.list <- sub("_[A-Z]_\\d+$", "", normalized.list)
      ...
      msg("DEBUG: Mapped ", sum(!is.na(list.features.entrez)), " list features to Entrez")
      initsbls <- doEntrez2SymbolMapping(list.features.entrez, paramSet$data.org, "entrez")
    }
    names(initsbls) <- analSet$list.features
    netData <- list(sizes=analSet$listSizes, proteinlist=initsbls);
    netName <- paste0(netNm, ".json");
  ...
  # Limit to top 20 pathways (after filtering by hits/total thresholds)
  if(nrow(fgseaRes) > 20) {
    msg("DEBUG: Final limit - keeping top 20 of ", nrow(fgseaRes), " pathways")
    fgseaRes <- fgseaRes[1:20, ]
  }
  ...
  if(mType == "network"){
    ...
    # CRITICAL: For GSEA network, use ALL ranked features (data.norm), not just significant ones (sig.mat)
    # rankedVec already has Entrez IDs as names (converted earlier)
    all.feature.ids <- names(rankedVec)
    msg("DEBUG: Network mode - using ", length(all.feature.ids), " features from rankedVec")
    msg("DEBUG: First 5 feature IDs (already Entrez): ", paste(head(all.feature.ids, 5), collapse=", "))

    # Convert Entrez IDs to gene symbols for display
    analSet$list.features <- doEntrez2SymbolMapping(all.feature.ids, paramSet$data.org, "entrez")
    msg("DEBUG: analSet$list.features first 5 symbols: ", paste(head(analSet$list.features, 5), collapse=", "))

    analSet <- SetListNms(dataSet);
    msg("DEBUG: Calling .prepareEnrichNet with type='list' for network generation")
    analSet <- .prepareEnrichNet(dataSet, netNm, "list", "mixed", analSet);
  ...
  paramSet$partialToBeSaved <- c(paramSet$partialToBeSaved, c(json.nm, "current_featureset.qs"))
  sink(json.nm)
  cat(json.mat);
  sink();
  ...
  imgSet$enrTables[["gsea"]]$current.featureset <- current.featureset;
  ...
  imgSet$enrTables[["gsea"]]$current.featureset.symb <- current.featureset.symb;
...
PlotGShm <-function(dataName="", cmpdNm="", IDs){
  ...
  if(anal.type == "onedata"){
    ...
    # Determine if data uses UniProt IDs
    data.ids <- rownames(dataSet$data.norm)
    is.uniprot <- mean(grepl("^[A-Z][0-9][A-Z0-9]{3}[0-9]", data.ids)) > 0.5

    if(is.uniprot && "uniprot" %in% colnames(gene.map)) {
      # Data IDs are UniProt, map UniProt -> Symbol
      subset <- dataSet$data.norm[which(doIdMappingGeneric(data.ids, gene.map, "uniprot", "symbol") %in% ids),]
    } else {
      # Data IDs are Entrez, map Entrez -> Symbol
      subset <- dataSet$data.norm[which(doIdMappingGeneric(data.ids, gene.map, "gene_id", "symbol") %in% ids),]
    }

    # Fallback: direct ID matching
    if(length(subset)<1){
      subset <- dataSet$data.norm[which(data.ids %in% ids),]
    }
    ...
    # Determine if data uses UniProt IDs
    data.ids <- rownames(dat)
    is.uniprot <- mean(grepl("^[A-Z][0-9][A-Z0-9]{3}[0-9]", data.ids)) > 0.5

    if(is.uniprot && "uniprot" %in% colnames(gene.map)) {
      # Data IDs are UniProt, map UniProt -> Symbol
      subset <- dat[which(doIdMappingGeneric(data.ids, gene.map, "uniprot", "symbol") %in% ids),]
    } else {
      # Data IDs are Entrez, map Entrez -> Symbol
      subset <- dat[which(doIdMappingGeneric(data.ids, gene.map, "gene_id", "symbol") %in% ids),]
    }

    # Fallback: direct ID matching
    if(length(subset)<1){
      subset <- dat[which(data.ids %in% ids),]
    }
  ...
  # Determine mapping column based on ID type
  subset.ids <- rownames(subset)
  is.uniprot.subset <- mean(grepl("^[A-Z][0-9][A-Z0-9]{3}[0-9]", subset.ids)) > 0.5
  map.col <- if(is.uniprot.subset && "uniprot" %in% colnames(gene.map)) "uniprot" else "gene_id"

  orig.gene.nms <- doIdMappingGeneric(subset.ids, gene.map, map.col, "symbol");
  ...
  if(anal.type == "onedata"){
    ...
    res.tbl$id <- doIdMappingGeneric(rownames(res.tbl), gene.map, map.col, "symbol");
    ...
    res.tbl <- res.tbl[which(rownames(res.tbl) %in% rownames(subset)),];
    rownames(res.tbl) <- doIdMappingGeneric(rownames(res.tbl), gene.map, map.col, "symbol");
  ...
  json.res <- list(
    data=subset,
    ids=doIdMappingGeneric(subset.ids, gene.map, map.col, "symbol"),
    entrez = subset.ids,
