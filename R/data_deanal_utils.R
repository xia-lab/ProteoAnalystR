...
.prepare.deseq <- function(dataSet, anal.type, par1, par2, nested.opt) {
  ...
  my.fun <- function() {
    ...
    parse_contrast_groups <- function(cstr) {
      ...
      if (length(comps) != 2) { AddErrMsg(paste("Invalid contrast format:", cstr)); return(0); }
      return(comps)
    }
    ...
    if (anal.type == "default") {
      ...
      if (!(ref %in% all_conditions)) {
        AddErrMsg(paste0("Reference level not found: ", ref)); return(0);
      }
      ...
      if (!all(comps %in% all_conditions)) {
        AddErrMsg(paste0("Invalid custom contrast: ", par1)); return(0);
      }
...
.perform_deqms <- function(dataSet, robustTrend = FALSE) {
  ...
  if (!requireNamespace("DEqMS", quietly = TRUE)) {
    AddErrMsg("DEqMS requires the 'DEqMS' package. Install with: BiocManager::install('DEqMS')");
    return(0);
  }
...
parse_contrast_groups <- function(contrast_str) {
  ...
  if (length(comps) != 2) { AddErrMsg(paste("Invalid contrast format:", contrast_str)); return(0); }
  return(comps)
}
