...
## Description: Heatmap wrapper functions for onedata, metadata, proteinlist
## Authors: 
...
SaveHeatmapJSON <- function(dataName="", fileName, displayOpt){
  ...
  if(!exists("my.prepare.heatmap.json")){ 
     compiler::loadcmp(paste0(resource.dir, "rscripts/ProteoAnalystR/R/utils_heatmap_table.Rc"));    
  }
...
SaveListHeatmapJSON <- function(dataName="", fileName){
  ...
  if(!exists("my.prepareListHeatmapJSON")){ 
     compiler::loadcmp(paste0(resource.dir, "rscripts/ProteoAnalystR/R/utils_heatmap_list.Rc"));    
  }
