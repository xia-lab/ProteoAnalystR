...
compute.ridgeline <- function(dataSet, imgNm = "abc", dpi=96, format="png", fun.type = "kegg", ridgeType = "ora", ridgeColor = "teal",rankOpt="fc", sigLevel = 0.05, pwNum=20, inx = 1){
  ...
  # For protein lists, degs.plot$entrez may contain Entrez IDs or UniProt IDs
  # depending on the analysis. Only convert if IDs are actually Entrez (numeric-like).
  if(anal.type == "proteinlist" && paramSet$data.idType == "uniprot") {
    # Check if IDs look like Entrez (numeric) or UniProt (alphanumeric like P12345, Q7Z7G0)
    sample.ids <- head(degs.plot$entrez, 10)
    ids.are.entrez <- all(grepl("^[0-9]+$", sample.ids))
    if(ids.are.entrez) {
