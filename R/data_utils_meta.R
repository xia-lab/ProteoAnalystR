...
SetupMetaStats <- function(BHth=0.05, paramSet, analSet){
  ...
  # meta.sig features
  metade.features <- rownames(meta.mat);
  
  # setup individual sig features & stats
  ...
  for(i in 1:length(analSet$inmex.ind)){
    ...
    # only choose the features that are also meta sig features from in
    # individual analysis for display
    de.res <- de.res[metade.features,];
  ...
  gains=metade.features[which(!(metade.features %in% deindst))];
  losses=deindst[which(!(deindst %in% metade.features))];
  all.de <- cbind(gene.ids %in% metade.features, gene.ids %in% deindst);
  colnames(all.de) <- c("Meta-DE", "Individual-DE");
  if(inmex.meta$id.type == "entrez"){ 
    names(metade.features) <- inmex.meta$gene.symbls[metade.features];
    names(gains) <- inmex.meta$gene.symbls[gains];
    names(losses) <- inmex.meta$gene.symbls[losses];
  }
  
  # de features from individual 
  de.len <- sapply(inmex.de, length);
  stat <- c(length(metade.features), de.len);
  ...
  if(inmex.meta$id.type == "entrez"){ # row name gene symbols
    metade.nms <- inmex.meta$gene.symbls[metade.features];
    res <- cbind(EntrezID=metade.features, Name=metade.nms,dat.mat);
  }else{
    res <- cbind(ID=metade.features, dat.mat);
  }
  analSet$meta.res.table <- res;
  fast.write(res, file=paste("meta_sig_features_", paramSet$inmex.method, ".csv", sep=""), row.names=F);
...
RestoreMetaData <- function(){
    ...
    #msg("[MetaQC] RestoreMetaData: restoring datasets from original matrix");
...
.update.datasets.from.meta <- function(meta.obj){
    ...
    for(dataName in sel.nms){
        ...
        #msg(paste("[MetaQC] Updated dataset", dataName, "with", ncol(adj), "samples"));
        RegisterData(dataSet);
    }
    return(1);
}
...
.finalizeBatchCorrection <- function(){
    ...
    inmex.meta <- qs::qread("inmex_meta.qs");
    #msg("[MetaQC] FinalizeBatchCorrection: updating datasets from merged matrix");
