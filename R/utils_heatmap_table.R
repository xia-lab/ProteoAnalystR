...
my.prepare.heatmap.json <- function(dataSet, displayOpt="sig"){
  ...
  if(displayOpt == "all"){
    ...
    # Sorting features by variance and selecting top 5000 if there are more than that
  ...
  if(dataSet$annotated){
    ...
    # Determine which column to use for matching based on what's in the rownames
    # If rownames are UniProt IDs, use uniprot/accession column; otherwise use gene_id (Entrez)
    match_col <- "gene_id";  # default to Entrez
    if ("uniprot" %in% colnames(gene.map)) {
      match_col <- "uniprot";
    } else if ("accession" %in% colnames(gene.map)) {
      match_col <- "accession";
    }

    anot.res <- doIdMappingGeneric(anot.id, gene.map, match_col, "symbol", "matrix")
