...
CheckMetaDataIntegrity <- function(){
  ...
  if(paramSet$meta.upload){
    ...
    for(i in 2:length(sel.nms)){
      ...
      # check and record if there is common features
      shared.nms <- intersect(shared.nms, rownames(dataSet$data.norm));
      if(length(shared.nms) < 10){
        msgSet$current.msg <- paste(sel.nms[i], "has less than 10 common features/probes from previous data sets");
        saveSet(msgSet, "msgSet");
        return(0);
      }
...
SanityAttachMeta(sel.nms, paramSet, msgSet);
  ...
  write.table(t(res), sep="\t", file="ProteoAnalyst_merged_data.txt", col.names=F, quote=FALSE);
  ...
  #msg("[MetaQC] Saved merged meta (raw snapshot)");
  if(!file.exists("inmex_meta.qs")){
  qs::qsave(inmex.meta, "inmex_meta.qs");
  }
  #msg("[MetaQC] Saved merged meta (current)");
...
PerformMetaEffectSize<- function(method="rem", BHth=0.05){
  ...
  # now keep only features with at least on sig (in one study or meta analysis)
...
PerformVoteCounting <- function(BHth = 0.05, minVote){
  ...
  vc.mat[,length(sel.nms)+1] <- rowSums(vc.mat);
...
PerformMetaMerge<-function(BHth=0.05){
  ...
  # calculate sig features for individual analysis
...
GetMetaResultfeaturesymbols<-function(){
  analSet <- readSet(analSet, "analSet");
