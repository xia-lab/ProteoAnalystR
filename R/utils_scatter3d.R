...
my.json.scatter <- function(dataSet, filenm="abc"){
  ...
  # If metatypes is NULL or empty, initialize with default metadata columns
  if(is.null(metatypes) || length(metatypes) == 0){
    if(!is.null(metadf) && ncol(metadf) > 0){
      # Create default metatypes - assume all are discrete unless numeric
      metatypes <- list()
      for(col_name in colnames(metadf)){
        # Check if column is numeric/continuous
        if(is.numeric(metadf[[col_name]])){
          metatypes[[col_name]] <- "cont"
        } else {
          metatypes[[col_name]] <- "disc"
        }
      }
    } else {
      # Fallback: create a minimal metatypes structure
      metatypes <- list(Group = "disc")
    }
  }

  # Ensure metadf is not NULL
  if(is.null(metadf)){
