...
my.enrich.net<-function(dataSet, netNm="abc", type="list", overlapType="mixed", analSet){
  ...
  # Static enrichment-map PNG (pathway nodes, protein-overlap edges) for the
  # report / dashboard, mirroring this interactive network. WfEmitEnrichNetwork
  # caches the plot inputs, draws via WfPlotEnrichNet (a file device under a
  # device guard so it never opens a quartz window), records a PlotEnrichNetReplot()
  # command so the figure is Refine-able, and emits the dedicated network manifest
  # (caption). netNm is "enrichNet_<lib>"; the figure is enrichnet_<lib>_static.
