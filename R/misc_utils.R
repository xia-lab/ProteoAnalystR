...
# Conditional message function that respects global debug flag
# Usage: msg("Some debug message") instead of message("Some debug message")
msg <- function(...) {
  if(exists(".show.debug.messages", envir = .GlobalEnv) &&
     isTRUE(get(".show.debug.messages", envir = .GlobalEnv))) {
    message(...)
  }
}

# Enable/disable debug messages globally
EnableDebugMessages <- function(enable = TRUE) {
  .show.debug.messages <<- enable
  if(enable) {
    message("[Debug] Debug messages ENABLED")
  } else {
    message("[Debug] Debug messages DISABLED")
  }
}

# Get current debug message status
GetDebugMessageStatus <- function() {
  if(exists(".show.debug.messages", envir = .GlobalEnv)) {
    return(get(".show.debug.messages", envir = .GlobalEnv))
  }
  return(FALSE)
}
...
RemoveDuplicates <- function(data, lvlOpt, quiet=T, paramSet, msgSet){
  ...
  if(sum(dup.inx) > 0){
    ...
    # OPTIMIZED: Use vectorized aggregate instead of loop
    # This is 10-100x faster for datasets with many duplicates
    agg.data <- as.data.frame(data);
    agg.data$row_id <- all.nms;

    if(lvlOpt == "mean"){
      agg.result <- aggregate(. ~ row_id, data=agg.data, FUN=mean, na.rm=T);
    }else if(lvlOpt == "median"){
      agg.result <- aggregate(. ~ row_id, data=agg.data, FUN=median, na.rm=T);
    }else if(lvlOpt == "max"){
      agg.result <- aggregate(. ~ row_id, data=agg.data, FUN=max, na.rm=T);
    }else{ # sum
      agg.result <- aggregate(. ~ row_id, data=agg.data, FUN=sum, na.rm=T);
    }

    # Convert to matrix and restore structure
    rownames(agg.result) <- agg.result$row_id;
    agg.result$row_id <- NULL;
    uniq.data <- as.matrix(agg.result);
    # Ensure column names are preserved
    colnames(uniq.data) <- colnms;
    if(!quiet){
      if(paramSet$numOfLists == 1){
        ...
        msgSet$current.msg <- paste(msgSet$current.msg, paste0("<b>", dataSet$listInx, "</b> : ", length(data), " features;"), collapse="\n");
      }
    }
...
PerformHeatmapEnrichment <- function(dataName="", file.nm, fun.type, IDs){
  ...
  if (is_phospho) {
    ...
    res <- .performEnrichAnalysisPhospho(dataSet, file.nm, fun.type, gene.vec, "heatmap");
  } else {
...
GetListEnrGeneNumber <- function(){
  ...
  analSet$list.features <- all.enIDs;
...
InitEnrichmentNetwork <- function(dataName, type){
  ...
  # Check if there are any features for enrichment
  if(is.null(analSet$list.features) || length(analSet$list.features) == 0){
    msgSet$current.msg <- "No features found for enrichment network analysis. Please ensure your dataset contains valid features.";
    saveSet(msgSet, "msgSet");
    return(0);
  }
  ...
  if (is_phospho) {
    # For phospho data, analSet$list.features contains phosphosite IDs
...
PerformListEnrichmentView <- function(dataName="", file.nm, fun.type, netNm, IDs){
  ...
  if(length(gene.vec) == 0){
    msgSet$current.msg <- "No features provided for enrichment network analysis. Please provide a valid feature list.";
    saveSet(msgSet, "msgSet");
    return(0);
  }

  # Check if phospho data and use appropriate ID handling
  is_phospho <- (!is.null(paramSet$data.type) && paramSet$data.type == "phospho")

  if (!is_phospho) {
    # Regular data - gene.vec contains UniProt IDs, map to symbols for display
    # Use "uniprot" as ID type (not paramSet$data.idType which is initial upload type)
    sym.vec <- doEntrez2SymbolMapping(gene.vec, paramSet$data.org, "uniprot");
    names(gene.vec) <- sym.vec;
  }

  analSet$list.features <- gene.vec
  saveSet(analSet, "analSet");
  }
...
CalculateDEfeaturesetEnr <- function(nms, operation, refNm, filenm){
  paramSet <- readSet(paramSet, "paramSet");
  anal.type <- paramSet$anal.type;
  nms <- strsplit(nms, ";")[[1]];
...
.getDynLoadPath <- function() {
    if (exists("resource.dir")) {
        ...
        path <- "../../rscripts/ProteoAnalystR/R/src/ProteoAnalyst.so";
    }
...
saveSet <- function(obj=NA, set="", output=1){
     if(set == ""){
      ...
      # Update global variables AND in-memory cache
      if(set == "dataSet"){
        dataSet <<- obj;
      }else if(set == "analSet"){
        analSet <<- obj;
      ...
      if(exists(".use.memory.cache", envir = .GlobalEnv) &&
         isTRUE(get(".use.memory.cache", envir = .GlobalEnv))) {
        if(!exists(".set.cache", envir = .GlobalEnv)) {
          .set.cache <<- list()
        }
        cache <- get(".set.cache", envir = .GlobalEnv)
        cache[[set]] <- obj
...
readSet <- function(obj = NULL, set = "") {
  ...
  if(exists(".use.memory.cache", envir = .GlobalEnv) &&
     isTRUE(get(".use.memory.cache", envir = .GlobalEnv)) &&
     exists(".set.cache", envir = .GlobalEnv)) {
    cache <- get(".set.cache", envir = .GlobalEnv)
    if(!is.null(cache[[set]])) {
      return(cache[[set]])
    }
  }
...
#' Enable in-memory caching for readSet/saveSet
#'
#' When enabled, readSet will check memory cache before reading from disk,
#' and saveSet will update the cache. This can significantly speed up
#' repeated reads of the same objects (paramSet, analSet, etc.)
#'
#' @param enable Logical. TRUE to enable caching, FALSE to disable.
#' @export
EnableSetCache <- function(enable = TRUE) {
  .use.memory.cache <<- enable
  if(enable) {
    msg("[EnableSetCache] In-memory caching for readSet/saveSet is now ENABLED")
  } else {
    msg("[EnableSetCache] In-memory caching for readSet/saveSet is now DISABLED")
  }
}

#' Clear the in-memory cache for readSet/saveSet
#'
#' Removes all cached objects from memory. Use this if you need to force
#' re-reading from disk files.
#'
#' @param verbose Logical. If TRUE, print cache size before clearing.
#' @export
ClearSetCache <- function(verbose = TRUE) {
  if(exists(".set.cache", envir = .GlobalEnv)) {
    if(verbose) {
      cache <- get(".set.cache", envir = .GlobalEnv)
      msg(sprintf("[ClearSetCache] Clearing cache with %d objects: %s",
                      length(cache), paste(names(cache), collapse=", ")))
    }
    rm(".set.cache", envir = .GlobalEnv)
  } else {
    if(verbose) msg("[ClearSetCache] Cache is already empty")
  }
}
...
#'
#' @return List with cache status, size, and object names
#' @export
GetSetCacheInfo <- function() {
  enabled <- exists(".use.memory.cache", envir = .GlobalEnv) &&
             isTRUE(get(".use.memory.cache", envir = .GlobalEnv))
  ...
  if(exists(".set.cache", envir = .GlobalEnv)) {
    ...
    cache_bytes <- object.size(cache)
  } else {
    cache_size <- 0
    cache_names <- character(0)
    cache_bytes <- 0
  }
  ...
  list(
    ...
    size = cache_size,
    objects = cache_names,
    memory_mb = as.numeric(cache_bytes) / 1024^2
  )
}
...
readDataset <- function(dataName = "", quiet = FALSE) {
  ...
  if (length(dataName) == 0L || dataName == "" || is.na(dataName) || identical(dataName, "NA")) {
    ...
    paramSet <- readSet(paramSet, "paramSet")
    if (!is.null(paramSet$dataName) && nzchar(paramSet$dataName)) {
      dataName <- paramSet$dataName
      if (!quiet) msg("[readDataset] dataName was NA/empty; using paramSet$dataName=", dataName)
    } else {
...
IsPeptideQuant <- function(dataName = "") {
  ds <- readDataset(dataName, quiet = TRUE)
  if (is.null(ds)) {
    return(FALSE)
  }
  if (!is.null(ds$type) && ds$type == "peptide") {
    return(TRUE)
  }
  if (!is.null(ds$format) && grepl("peptide", ds$format, ignore.case = TRUE)) {
    return(TRUE)
  }
  FALSE
}
...
getFormatFilterMsg <- function(){
    msgSet <- readSet(msgSet, "msgSet");
    if (is.null(msgSet$prefilter.msg)) {
      return("")
    }
    return(msgSet$prefilter.msg);
}
