...
MapListIds <- function(listNm, geneIDs, org, idType){
  ...
  # Create symbol.map.qs for network visualization (includes uniprot column)
  # Get unique mapped gene_ids
  unique.mapped <- unique(combined.mapping.df[combined.mapping.df$gene_id != "NA", ])
  if (nrow(unique.mapped) > 0) {
    # Query gene database for symbol and name
    tblNm <- getEntrezTableName(org, "entrez");
    symbol.map <- queryGeneDB(tblNm, org);
    if (!is.null(symbol.map) && is.data.frame(symbol.map)) {
      # Filter to only genes we have
      symbol.map <- symbol.map[symbol.map$gene_id %in% unique.mapped$gene_id, ];
      # Add uniprot column
      symbol.map <- .addUniprotColumn(symbol.map, idType, unique.mapped$orig, unique.mapped$gene_id, paramSet);
      # Save for network visualization
      saveDataQs(symbol.map, "symbol.map.qs", "proteinlist", listNm);
    }
  }
...
row.num <- nrow(dat);
  ...
  paramSet$anal.type <- "proteinlist";
...
MapMultiListIds <- function(listNm, org, geneIDs, type){
  ...
  for(i in seq_along(listNms)){
    ...
    mapped <- GeneAnotDB[!GeneAnotDB$unmapped, c("orig", "gene_id")];
    mapped$symbol <- doEntrez2SymbolMapping(mapped$gene_id, org, type);
  ...
  # Create symbol.map.qs for network visualization (includes uniprot column)
  # Get unique mapped gene_ids
  unique.mapped <- unique(merged.mapping[merged.mapping$gene_id != "NA", ])
  if (nrow(unique.mapped) > 0) {
    # Query gene database for symbol and name
    tblNm <- getEntrezTableName(org, "entrez");
    symbol.map <- queryGeneDB(tblNm, org);
    if (!is.null(symbol.map) && is.data.frame(symbol.map)) {
      ...
      symbol.map <- symbol.map[symbol.map$gene_id %in% unique.mapped$gene_id, ];
      # Add uniprot column
      symbol.map <- .addUniprotColumn(symbol.map, type, unique.mapped$orig, unique.mapped$gene_id, paramSet);
      # Save for network visualization
      saveDataQs(symbol.map, "symbol.map.qs", "proteinlist", listNm);
    }
  }
