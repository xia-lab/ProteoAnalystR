...
.onAttach <- function (libname, pkgname){
  .on.public.web <<- FALSE;
  k1 <- paste("ProteoAnalystR",
              utils::packageVersion( "ProteoAnalystR"),
...
.machine.lib.paths <- function(onWeb=T, dataPath="data/"){
  ...
  }else if(file.exists("/home/le/sqlite/proteoanalystdatabase/")){
    sqlite.path <- "/home/le/sqlite/proteoanalystdatabase/"; #le local
  ...
  }else if(file.exists("/Users/gordonli/sqlite/")){ # gordon local
    sqlite.path <- "/Users/gordonli/sqlite/";
  }else{
  ...
  out["jointpa.lib.path"] <- list(jointpa.lib.path);
  ...
  if(!onWeb) {
    ...
    out$sqlite.path <- if (nzchar(sqlite.path)) sqlite.path else paste0(getwd(), "/");
    out$lib.path <- "https://www.proteoanalyst.ca/ProteoAnalyst/resources/data/";
  }else{
...
Init.Data <-function(onWeb=T, dataPath="data/", default.dpi=72){
  ...
  print("called proteoanalyst init!");
...
.prepareListSeeds <- function(){
    ...
    write(paste("#DataSet:", dataSet$name),file="sig_features.txt",append=TRUE);
    write.table(dataSet$sig.mat, file="sig_features.txt", append=TRUE);
    ...
    #seed.features <<- unique(meta.gene.vec);
...
UpdateSubnetStats <- function(){
    ...
    # Also update global net.stats for GetNetsNameString()
    net.stats <<- net.stats;
    saveSet(analSet, "analSet");
}
